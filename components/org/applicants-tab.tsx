import { Check, RotateCcw, SearchX, X } from "lucide-react";

import { DEMO_APPLICANT_ID } from "@/lib/demoStore";
import { APPLICANTS, displayFont, initials, matchColor, TOTAL_APPLIED, type Decision } from "./data";
...
export function ApplicantsTab({
  ...
  const rows = query
    ? APPLICANTS.filter((a) =>
        [a.name, a.faculty, a.level, a.reason].some((f) => f.toLowerCase().includes(query))
      )
    : APPLICANTS;

  // Real average of the listed applicants, rather than a hardcoded figure.
  const avgMatch = Math.round(APPLICANTS.reduce((sum, a) => sum + a.match, 0) / APPLICANTS.length);
  ...
  return (
    <div className={styles.tabIn}>
        ...
        <StatCard
          ...
          value={
            <>
              ...
              <span className="text-[15px] text-[#9aa39c]">/ {totalSlots}</span>
            </>
          }
        />
        <div
      ...
      <div className="overflow-hidden rounded-[22px] border border-[#E9EFE6] bg-white shadow-[0_10px_30px_-22px_rgba(28,61,39,.4)]">
        ...
        {rows.length === 0 && (
          <div className={`flex flex-col items-center gap-2 px-6 py-14 text-center ${styles.fadeUpFast}`}>
            <SearchX className="size-7 text-[#C4D3BC]" strokeWidth={1.8} />
            <div className="text-[13.5px] font-semibold text-[#3f4a43]">No applicants match “{search}”</div>
            <div className="text-[12px] text-[#8a938b]">Try a name, faculty or level.</div>
          </div>
        )}
        ...
        {rows.map((a) => {
          ...
          return (
            <div
              key={a.id}
              className={`${rowClass} ${styles.fadeUpFast} items-center border-t border-[#EEF3EC] px-[22px] py-4 transition-colors ${isLive ? "bg-[#F3FBED]" : "hover:bg-[#FAFCF8]"}`}
            >
                ...
                {!decision ? (
                  <>
                    <button
                      ...
                      className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-[11px] border-[1.5px] border-[#F0D5D5] bg-white text-[#D2453F] transition-all hover:bg-[#FDECEC] active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="flex h-9 cursor-pointer items-center gap-1.5 rounded-[11px] px-[15px] text-[12.5px] font-semibold text-[#0f2a17] transition-all hover:brightness-105 active:scale-95"
                  ...
                  <>
                    <span
                      className={`rounded-[11px] px-3.5 py-2 text-[12px] font-semibold ${styles.popIn}`}
                      style={{
                    ...
                    <button
                      type="button"
                      onClick={() => onUndo(a.id)}
                      aria-label={`Undo decision for ${a.name}`}
                      title="Undo"
                      className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-[10px] text-[#9aa39c] transition-all hover:bg-[#EEF3EC] hover:text-[#23472D] active:scale-95"
                    >
